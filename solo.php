...
WHERE SoloMF = 'Solo'");
$sql2->execute();
$sg = $sql2->fetchAll(PDO::FETCH_OBJ);
print_r($res);
?>
...
<body>
    <div class="page page-light">
        <div class="wrapper">
            ...
            <div class="page">
                <div class="wrapper">
                <h2 class="title-h2">Релизы соло исполнителей</h2>
                ...
                $sql2 = $pdo->prepare("SELECT * FROM Albums
                LEFT JOIN Singers on Singers.SingerName = Albums.SingerName
                WHERE SoloMF = 'Solo' ORDER BY ReliseDate");
                $sql2->execute();
                $sg = $sql2->fetchAll(PDO::FETCH_OBJ);
                }else{
                $sql2 = $pdo->prepare("SELECT * FROM Albums
                LEFT JOIN Singers on Singers.SingerName = Albums.SingerName
                WHERE SoloMF = 'Solo' ORDER BY ReliseDate DESC");
                $sql2->execute();
                $sg = $sql2->fetchAll(PDO::FETCH_OBJ);
                }
                ?>
                ...
                <div class="latest-releases">
                    <?php foreach($sg as $sgroup):?>
                    <div class="release">
                        <img src="img/solo/<?php echo $sgroup->ImgName?>" alt="Release">
                        <p class="release-title"><?php echo $sgroup->SingerName?> - <?php echo $sgroup->AlbName?></p>
                        <p class="release-price"><?php echo $sgroup->AlbPrice?> BYN</p>
                        <br>
                    </div>
                    <?php endforeach?>
               </div>
